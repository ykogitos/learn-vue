  ...
  <fieldset :class="{ required: required }">
    ...
    <div class="flex flex-row flex-align-center" role="radiogroup">
      <div class="input-radio" v-for="v in selectedRadios" :key="v.id">
        ...
        <input type="radio" :id="v.id" :name="name" :value="v.value" :aria-checked="v.selected" :required="required"
          v-model="childValue" @click="selectedId = v.id" @input="emit('update:value', $event)" />
      </div>
    </div>
  </fieldset>
</template>

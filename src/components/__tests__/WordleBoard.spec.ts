...
describe('WordleBoard', () => {
  ...
  async function playerTypesGuess(guess: string) {
    await wrapper.find('input.wordle[type=text]').setValue(guess)
  }

  async function playerPressesEnter() {
    await wrapper.find('input.wordle[type=text]').trigger('keydown.enter')
  }
  ...
  describe('Player input', () => {
    ...
    test.skip('non-letter characters do not render on the screen while being typed', async () => {
      ...
      // console.log('=>', wrapper.find<HTMLInputElement>("input.wordle[type=text]").element.innerHTML)
      expect(wrapper.find<HTMLInputElement>('input.wordle[type=text]').element.value).toEqual('')
    })
